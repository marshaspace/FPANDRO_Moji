...
import com.moji.v1.databinding.FragmentHistoryCalendarBinding
...
class HistoryCalendarFragment : Fragment() {
    ...
    private var _binding: FragmentHistoryCalendarBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentHistoryCalendarBinding.inflate(inflater, container, false)
        return binding.root
    }
        ...
        binding.calendarView.dayBinder = object : MonthDayBinder<DayViewContainer> {
            ...
            override fun bind(container: DayViewContainer, data: CalendarDay) {
                ...
                if (data.position == DayPosition.MonthDate) {
                    ...
                    container.tvDay.setOnClickListener { showEntries(key) }
                } else {
                    container.tvDay.setTextColor(
                        ContextCompat.getColor(requireContext(), R.color.black)
                    )
    ...
    private fun showEntries(dateKey: String?) {
        val entries = if (dateKey == null) DummyData.entries
        else DummyData.entries.filter { it.dateKey == dateKey }

        if (entries.isEmpty()) {
            binding.rvCalendarEntries.visibility = View.GONE
            binding.tvEmpty.visibility = View.VISIBLE
        } else {
            binding.rvCalendarEntries.visibility = View.VISIBLE
            binding.tvEmpty.visibility = View.GONE
            binding.rvCalendarEntries.layoutManager = LinearLayoutManager(requireContext())
            binding.rvCalendarEntries.adapter = JournalAdapter(entries) {}
        }
    }
